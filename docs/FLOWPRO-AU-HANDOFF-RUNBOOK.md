...
> - **Section 5 is now packaged and ready to send**: `n8n-workflows/handover-package/` (zip: `n8n-workflows/FlowPro-Handover-TotalAI-2026-08.zip`) — standalone workflows **updated with all CRM-independent fixes** (E.164 SMS, PDF retry+label, error-visible FlowProIssues runs, sub-error-tolerant parent), all CRM references stripped, plus setup guide, operator runbook, expectations doc, 408-row address-reference CSV, cutover checklist, and the **Smart-Matching Service appendix**. Send the memo separately from the zip.
> - **Commercial structure (2026-08-05, revised after the Selva call prep):** cutover is an **in-place server transfer** (TotalAI takes over the UpCloud/n8n instance — nothing rebuilt; key steps: n8n owner-login handoff, SSH keys, domain off badou.ai **with OAuth-redirect re-registration**, upstream account ownership checks). **$1,750 USD settlement is unconditional** (memo §6). The brain is offered as the **Smart-Matching Service: $150 USD/mo locked for TotalAI (std $200), month-to-month, optional** (memo §7 + package appendix `50-…`): 30-day post-cutover decision window (connected free), TotalAI cancel 30d / Ady sunset ≥90d, documented standalone fallback either way (~9-in-10 vs ~6-in-10 automation, measured). No A/B bundling with the settlement — the service is separate, forward-looking, his choice.
