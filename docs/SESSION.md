...
- **✅ GO-LIVE EXECUTED 2026-08-05 22:30–22:50 UTC (Fable 5, user-approved):**
  ...
  4. **Monitoring:** first v2 execution pending traffic at handoff-time; verify Resolve/Learn fire and `flowpro_site_map.max(updated_at)` moves with each job. Expect red rate ~43% → ~10%. **Daily habit: check `SELECT max(updated_at) FROM flowpro_site_map` alongside mirror freshness — a frozen memory with green executions was the two-week blind spot.**
