...
## 💼 2026-08-05 — Handoff restructured: in-place server transfer + Smart-Matching Service offer

Prep for the Selva call reshaped the exit (details in `FLOWPRO-AU-HANDOFF-RUNBOOK.md` banner + package):
- **Cutover = in-place transfer** of the UpCloud/n8n instance (Selva already manages UpCloud; he gets the n8n owner login + SSH; moves the hostname off badou.ai — OAuth redirects must be re-registered). No rebuild; system transfers running.
- **$1,750 USD settlement is UNCONDITIONAL** (memo §6 rewritten) — settles 3+ months of unpaid support; not offset by anything.
- **The brain = "FlowPro Smart-Matching Service"**: optional vendor subscription, **$150 USD/mo locked** (std $200; flat, incl. auto-create), month-to-month, TotalAI-cancel 30d / Ady-sunset ≥90d, email-only support, API-boundary ("if /resolve answers, my side is healthy"). 30-day post-cutover decision window (stays connected). Declining → documented ~30-min standalone fallback (in the package). Measured levels: ~9-in-10 vs ~6-in-10 automated. Memo §7 added; package appendix `50-smart-matching-service/` written to survive pushback (why it exists, why separate IP, anticipated questions). TotalAI = de-facto first paying customer of the engine Jeffrey (US HOA product) will run.
- Package re-zipped (`FlowPro-Handover-TotalAI-2026-08.zip`, 12 files, secret-swept clean).
...
- **Learn-route hardening folded in (2026-08-05, after CTO review):** `app/api/flowpro/learn/route.ts` now coerces numeric-string `siteId`/`customerId`/`confidence` (Simpro can return a fresh site ID as a string — would have 400'd the created-path learn once v2 resumes), while `source` stays a **strict** enum (normalized for case/whitespace, but an unknown label is a sender bug that must surface, not be absorbed into the trust field). Verified: tsc clean + local handler smoke of all four validation branches. NOTE for the CTO: this supersedes the equivalent change staged in their separate checkout — theirs can be dropped to avoid a merge conflict.
- **✅ GO-LIVE EXECUTED 2026-08-05 22:30–22:50 UTC (Fable 5, user-approved):**
  1. **Deployed** — merged `flowpro-sibling-safety` → main (`b1c0078`), pushed to `origin` (`ARTIST-AI-TECH/totalai-crm` — NOTE: the only remote now; Netlify follows it, confirmed live ~75s after push). All five live probes pass: 41-vs-41a → clean match to the exact record; 99/615 → review with `unitMismatch` siblings; 2/17 Leadenhall → clean fuzzy match (minGap unblocked); string-`siteId` accepted by `/learn`.
  2. **Backfill run** (validated, `lib/flowpro/backfill.ts` on the new matcher): 403 site mappings seeded, 111 dup-keys collapsed, **6 historical mis-filings refused** (12 Broad/Broadway, 3× 6 Pym Belair/Croydon Park, 11 Cambridge, 212 Tarni). `flowpro_site_map` = 408 rows; "19 Third Street, Bowden" verified as instant L0 `hit`.
  3. **Re-pointed production** — `PPG-create-job`'s Call node → `WXihj3biRDzFT8no` (v2 — CRM resolution) via n8n API PUT; fresh GET confirms active=true, all credentials intact (Outlook trigger/move, Twilio), `staticData` preserved. **Rollback if ever needed:** point the Call node back to `BC9zzabGVrI6nMVH` (v1).
  4. **✅ VALIDATED on first live order (2026-08-06 00:06 UTC, exec 3454):** TAPI-007472 "29 Allinga AV, Glenside" → Resolve `match` (fuzzy, score 1, site 3695) → job created → PDF attached → Tapi accepted → **Learn wrote** (`29 allinga avenue glenside`, source `auto_match`, map now 409 rows). The full v2 chain incl. the learning loop is live again after 13 days dark. Expect red rate ~43% → ~10%. **Daily habit: check `SELECT max(updated_at) FROM flowpro_site_map` alongside mirror freshness — a frozen memory with green executions was the two-week blind spot.**
