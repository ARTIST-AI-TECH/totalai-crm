...
- **The brain = "FlowPro Smart-Matching Service"**: optional vendor subscription, **$200 USD/mo** flat (incl. auto-create), month-to-month, TotalAI-cancel 30d / Ady-sunset ≥90d, email-only support, API-boundary ("if /resolve answers, my side is healthy"). 30-day post-cutover decision window (stays connected). Declining → documented ~30-min standalone fallback (in the package). Measured levels: ~9-in-10 vs ~6-in-10 automated. Memo §7 added; package appendix `50-smart-matching-service/` written to survive pushback (why it exists, why separate IP, anticipated questions). TotalAI = de-facto first paying customer of the engine Jeffrey (US HOA product) will run.
