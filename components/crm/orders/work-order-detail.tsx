          ...
          {(order as any).smsSent && (
            <div>
              ...
              <p className={`text-lg font-semibold ${
                (order as any).smsStatus === 'delivered' ? 'text-green-600' :
                (order as any).smsStatus === 'sent' ? 'text-green-600' :
                (order as any).smsStatus === 'failed' ? 'text-red-600' :
                'text-yellow-600'
              }`}>
                {(order as any).smsStatus === 'delivered' ? '✓ Delivered' :
                 (order as any).smsStatus === 'sent' ? '✓ Sent' :
                 (order as any).smsStatus === 'failed' ? '✗ Failed' :
                 (order as any).smsStatus === 'queued' ? '⏳ Queued' :
                 '✓ Sent'}
              </p>
